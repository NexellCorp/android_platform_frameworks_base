...
final class ActivityStack {
    ...
    // psw0523 add for MultiWindow
    final ArrayList<ActivityRecord> mMultiWindowActivities = new ArrayList<ActivityRecord>();
    int mDefaultActivityIndex = 0;
    int mOtherActivityIndex = 0;
    int mLastActivityRecordIndex = 0;
    ActivityRecord mFirstActivity = null;
    ActivityRecord mSecondActivity = null;
    ActivityRecord mThirdActivity = null;
    ActivityRecord mFourthActivity = null;
    ActivityRecord mBackupDefaultActivity = null;
    ActivityRecord mBackupFirstActivity = null;
    ActivityRecord mBackupSecondActivity = null;
    final ArrayList<ActivityRecord> mActivityStackForDefault = new ArrayList<ActivityRecord>();
    final ArrayList<ActivityRecord> mActivityStackForSecond = new ArrayList<ActivityRecord>();
    // for restore default Activity
    ActivityRecord mPrevDefaultActivity = null;
    // for force resume prev activity
    ActivityRecord mForceResumeActivity = null;
    ...
    final class ActivityStackHandler extends Handler {
        //public Handler() {
        //    if (localLOGV) Slog.d(TAG, "Handler started!");
        //}
    ...
    void minimalResumeActivityLocked(ActivityRecord r) {
        ...
        if (DEBUG_STATES) Slog.d(TAG, "Moving to RESUMED: " + r
                + " (starting new instance)");
        r.stopped = false;

        // MULTIWINDOW
        // Slog.d(TAG, "=====> minimalResumeActivityLocked() --> CHECK getMultiWindowEnabled");
    ...
    boolean checkReadyForSleepLocked() {
        if (mResumedActivity != null) {
            ...
            if (DEBUG_PAUSE) Slog.d(TAG, "Sleep needs to pause " + mResumedActivity);
            if (DEBUG_USER_LEAVING) Slog.d(TAG, "Sleep => pause with userLeaving=false");
            startPausingLocked(false, true, false, false);
            return true;
        }
        if (mPausingActivity != null) {
            // Still waiting for something to pause; can't sleep yet.
            if (DEBUG_PAUSE) Slog.d(TAG, "Sleep still waiting to pause " + mPausingActivity);
            return true;
        }
    ...
    final boolean startPausingLocked(boolean userLeaving, boolean uiSleeping, boolean resuming,
        ...
        if (DEBUG_STATES) Slog.d(TAG, "Moving to PAUSING: " + prev);
        else if (DEBUG_PAUSE) Slog.d(TAG, "Start pausing: " + prev);
        ...
        if (prev.app != null && prev.app.thread != null) {
            if (DEBUG_PAUSE) Slog.d(TAG, "Enqueueing pending pause: " + prev);
            try {
        ...
        if (mPausingActivity != null) {
            ...
            if (!uiSleeping) {
                ...
                if (DEBUG_PAUSE) Slog.d(TAG, "Key dispatch not paused for screen off");
            }
            ...
            if (dontWait) {
                ...
                if (DEBUG_PAUSE) Slog.d(TAG, "Waiting for pause to complete...");
                return true;
            }
            ...
            if (DEBUG_PAUSE) Slog.d(TAG, "Activity not running, resuming next.");
            if (!resuming) {
    ...
    final int pauseActivity(ActivityRecord r, boolean finishing) {
        ...
        mResumedActivity = r;
        // end test
        // r.finishing = finishing;
        r.finishing = true;

        if (r.app != null && r.app.thread != null) {
            try {
                r.app.thread.schedulePauseActivity(r.appToken, r.finishing, false, r.configChangeFlags, false);
            } catch (Exception e) {
                Slog.e(TAG, "Error: schedulePauseActivity()");
                mPausingActivity = null;
            }
        } else {
            mPausingActivity = null;
        ...
        if (!finishing) {
            backupMultiWindowActivity(r);
        } else {
            if (mPausingActivity == mFirstActivity) {
                // pause all activities in mActivityStackForDefault
                int stackCount = mActivityStackForDefault.size();
                for (int i = 0; i < stackCount; i++) {
                    ActivityRecord sr = mActivityStackForDefault.get(i);
                    if (sr.app != null && sr.app.thread != null) {
                        try {
                            Slog.d(TAG, "pauseActivity in mActivityStackForDefault --> " + sr);
                            sr.finishing = true;
                            sr.app.thread.schedulePauseActivity(sr.appToken, sr.finishing, false, sr.configChangeFlags, false);
                            mPausingActivity = sr;
                            sr.pauseKeyDispatchingLocked();
                            completePauseLocked(false);
                        } catch (Exception e) {
                            Slog.e(TAG, "Error: schedulePauseActivity() for mActivityStackForDefault --> " + sr);
                        }
                    }
                }
                mActivityStackForDefault.clear();
            } else if (mPausingActivity == mSecondActivity) {
                // pause all activities in mActivityStackForSecond
                int stackCount = mActivityStackForSecond.size();
                for (int i = 0; i < stackCount; i++) {
                    ActivityRecord sr = mActivityStackForSecond.get(i);
                    if (sr.app != null && sr.app.thread != null) {
                        try {
                            Slog.d(TAG, "pauseActivity in mActivityStackForSecond --> " + sr);
                            sr.finishing = true;
                            sr.app.thread.schedulePauseActivity(sr.appToken, sr.finishing, false, sr.configChangeFlags, false);
                            mPausingActivity = sr;
                            sr.pauseKeyDispatchingLocked();
                            ...
                            Slog.e(TAG, "Error: schedulePauseActivity() for mActivityStackForSecond --> " + sr);
                        }
                    }
                }
    ...
    final void removeMultiWindowActivity(ActivityRecord r) {
        synchronized(mService) {
            if (mFirstActivity == r) {
    ...
    // psw0523 add for AVN MultiWindow
    public int getLastIndexOfActivityRecord() {
        mLastActivityRecordIndex++;
        return mLastActivityRecordIndex;
    }

    // psw0523 add for AVN MultiWindow
    final ActivityRecord getMultiWindowActivityByIndex(int index) {
        int i;
        ActivityRecord r = null;
        for (i = 0; i < mMultiWindowActivities.size(); i++) {
            r = mMultiWindowActivities.get(i);
            if (r.mIndex == index)
                return r;
        }
    ...
    // psw0523 add for AVN MultiWindow
    final void setMultiWindowAppToken() {
        mWindowManager.clearMultiWindowAppToken();
        if (mFirstActivity != null)
            mWindowManager.addMultiWindowAppToken(mFirstActivity.appToken, mStackId, 0);
        if (mSecondActivity != null)
            mWindowManager.addMultiWindowAppToken(mSecondActivity.appToken, mStackId, 1);
    }
    ...
    final void updateMultiWindowActivities(ActivityRecord r, boolean isStackRestore) {
        ...
        synchronized(mService) {
            ...
            if (mFirstActivity == null) {
                ...
                if (isSameGroupActivity(r, mFirstActivity)) {
                    ...
                    if (!isStackRestore && !mActivityStackForDefault.contains(mFirstActivity)) {
                        mActivityStackForDefault.add(mFirstActivity);
                    }
                    Slog.d(TAG, "Change First by Same Task: from " + mFirstActivity + " to " + r);
                    // add for resume prev activity
                    if (isStackRestore && mSecondActivity != null) {
                        Slog.d(TAG, "set mForceResumeActivity to " + r);
                        mForceResumeActivity = r;
                    }
                    mFirstActivity = r;
                } else {
                    if (r.mIndex < mFirstActivity.mIndex) {
                        Slog.d(TAG, "Change First by index: from " + mFirstActivity + " to " + r);
                        mSecondActivity = mFirstActivity;
                        mFirstActivity = r;
                    } else {
                        ...
                        if (mSecondActivity != null) {
                            if (isSameGroupActivity(r, mSecondActivity)) {
                                ...
                                Slog.d(TAG, "Change Second by Same Task: from " + mSecondActivity + " to " + r);
                            } else {
                                Slog.d(TAG, "pauseActivity prev SecondActivity --> " + mSecondActivity);
                                pauseActivity(mSecondActivity, true);
                            }
                        }
                        mSecondActivity = r;
                    }
            ...
            //
            // if (forceStart) {
            //     Slog.d(TAG, "start prev activity --> " + r);
            //     startPrevActivity(r);
            // }
        }
    }

    final void changeLeftRight() {
        Slog.d(TAG, "changeLeftRight ----> ");
        synchronized(mService) {
            if (mFirstActivity != null && mSecondActivity != null) {
                int index = mFirstActivity.mIndex;
                ActivityRecord r = mFirstActivity;
                mFirstActivity.mIndex = mSecondActivity.mIndex;
                mSecondActivity.mIndex = index;
                
                mFirstActivity = mSecondActivity;
                mSecondActivity = r;

                // change mActivityStackForDefault, mActivityStackForSecond
                if (mActivityStackForDefault.size() > 0 || mActivityStackForSecond.size() > 0) {
                    ArrayList<ActivityRecord> tmpArray = (ArrayList<ActivityRecord>)mActivityStackForDefault.clone();
                    int i;
                    int size;

                    mActivityStackForDefault.clear();
                    size = mActivityStackForSecond.size();
                    for (i = 0; i < size; i++) {
                    ...
                    size = tmpArray.size();
                    for (i = 0; i < size; i++) {
                        mActivityStackForSecond.add(tmpArray.get(i));
                    }
                    tmpArray.clear();
                }
    ...
    final void activityPausedLocked(IBinder token, boolean timeout) {
        ...
        if (r != null) {
            ...
            if (mPausingActivity == r) {
                if (DEBUG_STATES) Slog.d(TAG, "Moving to PAUSED: " + r
    ...
    final void activityStoppedLocked(ActivityRecord r, Bundle icicle,
        ...
        if (!r.stopped) {
            if (DEBUG_STATES) Slog.d(TAG, "Moving to STOPPED: " + r + " (stop complete)");
            mHandler.removeMessages(STOP_TIMEOUT_MSG, r);
            r.stopped = true;
    ...
    private void completePauseLocked(boolean resumeNext) {
        ...
        if (DEBUG_PAUSE) Slog.d(TAG, "Complete pause: " + prev);

        if (prev != null) {
            prev.state = ActivityState.PAUSED;
            if (prev.finishing) {
                if (DEBUG_PAUSE) Slog.d(TAG, "Executing finish of activity: " + prev);
                prev = finishCurrentActivityLocked(prev, FINISH_AFTER_VISIBLE, false);
            } else if (prev.app != null) {
                if (DEBUG_PAUSE) Slog.d(TAG, "Enqueueing pending stop: " + prev);
                if (prev.waitingVisible) {
                    prev.waitingVisible = false;
                    mStackSupervisor.mWaitingVisibleActivities.remove(prev);
                    if (DEBUG_SWITCH || DEBUG_PAUSE) Slog.d(
                ...
                if (prev.configDestroy) {
                    ...
                    if (DEBUG_PAUSE) Slog.d(TAG, "Destroying after pause: " + prev);
                    ...
                    if (mStackSupervisor.mStoppingActivities.size() > 3 ||
                        ...
                        if (DEBUG_PAUSE) Slog.d(TAG, "To many pending stops, forcing idle");
                ...
                if (DEBUG_PAUSE) Slog.d(TAG, "App died during pause, not stopping: " + prev);
                prev = null;
            }
            mPausingActivity = null;
        }
            ...
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                ...
                if (!behindFullscreen || r.mLaunchTaskBehind) {
                    ...
                    if (r.app == null || r.app.thread == null) {
                        ...
                        // at this point.
                        if (DEBUG_VISBILITY) Slog.d(TAG, "Start and freeze screen for " + r);
                        if (r != starting) {
                        ...
                        // else to do here.
                        if (DEBUG_VISBILITY) Slog.d(TAG, "Skipping: already visible at " + r);
                    ...
                    if (r.fullscreen) {
                        ...
                        if (DEBUG_VISBILITY) Slog.d(TAG, "Fullscreen: at " + r);
                        behindFullscreen = true;
                    } else if (!isHomeStack() && r.frontOfTask && task.isOverHomeStack()) {
                        if (DEBUG_VISBILITY) Slog.d(TAG, "Showing home: at " + r);
                        behindFullscreen = true;
                    }
                } else {
                    if (DEBUG_VISBILITY) Slog.d(
                    ...
                    if (r.visible) {
                        if (DEBUG_VISBILITY) Slog.d(TAG, "Making invisible: " + r);
                        try {
                            boolean invisible = false;
                            if (mStackSupervisor.getMultiWindowEnabled()) {
                                if (isHomeStack()) {
                                    Slog.d(TAG, "home activity making to invisible");
                                    invisible = true;
                                }
                            ...
                            if (invisible) {
                                ...
                                switch (r.state) {
                                 ...
                                 Slog.d(TAG, "Don't invisible for MultiWindow, activity " + r);
                            }
                        } catch (Exception e) {
                            // Just skip on any failure; we'll make it
                            // visible when it next restarts.
                        ...
                        if (DEBUG_VISBILITY) Slog.d(TAG, "Already invisible: " + r);
                    }
                }
            }
        }
    ...
    final void startActivityLocked(ActivityRecord r, boolean newTask,
        ...
        if (!isHomeStack() || numActivities() > 0) {
            ...
            // psw0523 add for AVN MultiWindow
            if (mStackSupervisor.getMultiWindowEnabled())
                updateMultiWindowActivities(r, false);
    ...
    // psw0523 add for AVN MultiWindow
    final boolean isMultiWindowActivity(ActivityRecord r) {
        if (r == mFirstActivity ||
            r == mSecondActivity)
            return true;
        return false;
    ...
    // psw0523 add for AVN MultiWindow
    final void startPrevActivity(ActivityRecord r) {
        ...
        for (int i = numActivities - 1; i > rootActivityNdx; --i ) {
            ...
            if (!finishOnTaskLaunch
                ...
                if (bottom != null && target.taskAffinity != null
                    ...
                    if (DEBUG_TASKS) Slog.d(TAG, "Start pushing activity " + target
                ...
                for (int srcPos = start; srcPos >= i; --srcPos) {
                    ...
                    if (DEBUG_TASKS) Slog.d(TAG, "Pushing next activity " + p
        ...
        for (int i = numActivities - 1; i > rootActivityNdx; --i) {
            ...
            if (target.resultTo != null) {
                ...
                if (forceReset || finishOnTaskLaunch) {
                    ...
                    if (DEBUG_TASKS) Slog.d(TAG, "Finishing task at index " + start + " to " + i);
                    for (int srcPos = start; srcPos >= i; --srcPos) {
                        final ActivityRecord p = activities.get(srcPos);
                        if (p.finishing) {
                            continue;
                    ...
                    final int start = replyChainEnd >= 0 ? replyChainEnd : i;
                    if (DEBUG_TASKS) Slog.d(TAG, "Reparenting from task=" + affinityTask + ":"
                    ...
                    for (int srcPos = start; srcPos >= i; --srcPos) {
                        ...
                        if (DEBUG_ADD_REMOVE) Slog.i(TAG, "Removing and adding activity " + p
                                + " to stack at " + task,
                                new RuntimeException("here").fillInStackTrace());
                        if (DEBUG_TASKS) Slog.d(TAG, "Pulling activity " + p + " from " + srcPos
    ...
    void sendActivityResultLocked(int callingUid, ActivityRecord r,
        ...
        if (DEBUG_RESULTS) Slog.d(TAG, "Send activity result to " + r
    ...
    final void stopActivityLocked(ActivityRecord r) {
        ...
        if (r.app != null && r.app.thread != null) {
            ...
            try {
                r.stopped = false;
                if (DEBUG_STATES) Slog.d(TAG, "Moving to STOPPING: " + r
                        + " (stop requested)");
                ...
                r.stopped = true;
                if (DEBUG_STATES) Slog.d(TAG, "Stop failed; moving to STOPPED: " + r);
                r.state = ActivityState.STOPPED;
                if (r.configDestroy) {
    ...
    final boolean requestFinishActivityLocked(IBinder token, int resultCode,
        ...
        if (DEBUG_RESULTS || DEBUG_STATES) Slog.d(
        ...
        //         }
        //     }
        //     if (mActivityStackForSecond.size() > 0) {
        //         int lastIndex = mActivityStackForSecond.size() - 1;
    ...
    final void finishActivityResultsLocked(ActivityRecord r, int resultCode, Intent resultData) {
        ...
        if (resultTo != null) {
            if (DEBUG_RESULTS) Slog.d(TAG, "Adding result to " + resultTo
        ...
        else if (DEBUG_RESULTS) Slog.d(TAG, "No result destination from " + r);
    ...
    final boolean finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData,
        ...
        if (mResumedActivity == r) {
            ...
            if (DEBUG_VISBILITY || DEBUG_TRANSITION) Slog.d(TAG,
            ...
            if (mPausingActivity == null) {
                if (DEBUG_PAUSE) Slog.d(TAG, "Finish needs to pause: " + r);
                if (DEBUG_USER_LEAVING) Slog.d(TAG, "finish() => pause with userLeaving=false");
            ...
            if (DEBUG_PAUSE) Slog.d(TAG, "Finish not pausing: " + r);
            return finishCurrentActivityLocked(r, FINISH_AFTER_PAUSE, oomAdj) == null;
        } else {
            if (DEBUG_PAUSE) Slog.d(TAG, "Finish waiting for pause of: " + r);
        }
    ...
    final ActivityRecord finishCurrentActivityLocked(ActivityRecord r, int mode, boolean oomAdj) {
        ...
        if (DEBUG_STATES) Slog.d(TAG, "Moving to FINISHING: " + r);
        ...
        if (localLOGV) Slog.d(TAG, "Enqueueing pending finish: " + r);
    ...
    final void cleanUpActivityLocked(ActivityRecord r, boolean cleanServices,
        ...
        if (setState) {
            if (DEBUG_STATES) Slog.d(TAG, "Moving to DESTROYED: " + r + " (cleaning up)");
            r.state = ActivityState.DESTROYED;
            if (DEBUG_APP) Slog.d(TAG, "Clearing app during cleanUp for activity " + r);
            r.app = null;
        }
    ...
    private void removeActivityFromHistoryLocked(ActivityRecord r, String reason) {
        ...
        r.takeFromHistory();
        removeTimeoutsForActivityLocked(r);
        if (DEBUG_STATES) Slog.d(TAG, "Moving to DESTROYED: " + r + " (removed from history)");
        r.state = ActivityState.DESTROYED;
        if (DEBUG_APP) Slog.d(TAG, "Clearing app during remove for activity " + r);
        r.app = null;
    ...
    final void destroyActivitiesLocked(ProcessRecord owner, String reason) {
        ...
        for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
            ...
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                ...
                if (r.isDestroyable()) {
                    if (DEBUG_SWITCH) Slog.d(TAG, "Destroying " + r + " in state " + r.state
        ...
        for (int taskNdx = 0; taskNdx < mTaskHistory.size() && maxTasks > 0; taskNdx++) {
            ...
            for (int actNdx = 0; actNdx < activities.size(); actNdx++) {
                ...
                if (activity.app == app && activity.isDestroyable()) {
                    if (DEBUG_RELEASE) Slog.d(TAG, "Destroying " + activity
    ...
    final boolean destroyActivityLocked(ActivityRecord r, boolean removeFromApp, String reason) {
        if (DEBUG_SWITCH || DEBUG_CLEANUP) Slog.d(
        ...
        if (hadApp) {
            ...
            if (r.finishing && !skipDestroy) {
                if (DEBUG_STATES) Slog.d(TAG, "Moving to DESTROYING: " + r
                        + " (destroy requested)");
                r.state = ActivityState.DESTROYING;
                Message msg = mHandler.obtainMessage(DESTROY_TIMEOUT_MSG, r);
                mHandler.sendMessageDelayed(msg, DESTROY_TIMEOUT);
            } else {
                if (DEBUG_STATES) Slog.d(TAG, "Moving to DESTROYED: " + r + " (destroy skipped)");
                r.state = ActivityState.DESTROYED;
                if (DEBUG_APP) Slog.d(TAG, "Clearing app during destroy for activity " + r);
                r.app = null;
            }
        } else {
            ...
            if (r.finishing) {
                ...
                if (DEBUG_STATES) Slog.d(TAG, "Moving to DESTROYED: " + r + " (no app)");
                r.state = ActivityState.DESTROYED;
                if (DEBUG_APP) Slog.d(TAG, "Clearing app during destroy for activity " + r);
                r.app = null;
            }
    ...
    private void removeHistoryRecordsForAppLocked(ArrayList<ActivityRecord> list,
        ...
        int i = list.size();
        if (DEBUG_CLEANUP) Slog.d(
            TAG, "Removing app " + app + " from list " + listName
            + " with " + i + " entries");
        while (i > 0) {
            i--;
            ActivityRecord r = list.get(i);
            if (DEBUG_CLEANUP) Slog.d(TAG, "Record #" + i + " " + r);
            if (r.app == app) {
                if (DEBUG_CLEANUP) Slog.d(TAG, "---> REMOVING this entry!");
                list.remove(i);
                removeTimeoutsForActivityLocked(r);
            }
        }
            ...
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                ...
                if (r.app == app) {
                    ...
                    if (remove) {
                        ...
                        if (DEBUG_APP) Slog.d(TAG, "Clearing app during removeHistory for activity "
                                + r);
                        r.app = null;
                        r.nowVisible = false;
                        if (!r.haveState) {
    ...
    final void moveTaskToFrontLocked(TaskRecord tr, ActivityRecord source, Bundle options,
        ...
        if (DEBUG_TRANSITION) Slog.d(TAG, "Prepare to front transition: task=" + tr);
        if (source != null &&
                (source.intent.getFlags()&Intent.FLAG_ACTIVITY_NO_ANIMATION) != 0) {
            mWindowManager.prepareAppTransition(AppTransition.TRANSIT_NONE, false);
            ActivityRecord r = topRunningActivityLocked(null);
        ...
        // Why add this?
        // if (mStackSupervisor.getMultiWindowEnabled()) {
        //     Slog.d(TAG, "moveTaskToFrontLocked: call updateMultiWindowActivities --> " + source);
        //     updateMultiWindowActivities(source, false);
        // }
    ...
    final boolean ensureActivityConfigurationLocked(ActivityRecord r,
        ...
        if (mConfigWillChange) {
            if (DEBUG_SWITCH || DEBUG_CONFIGURATION) Slog.d(TAG,
        ...
        if (DEBUG_SWITCH || DEBUG_CONFIGURATION) {
            Slog.d(TAG, "Checking to restart " + r.info.name + ": changed=0x"
        ...
        if (r.app != null && r.app.thread != null) {
            try {
                if (DEBUG_CONFIGURATION) Slog.d(TAG, "Sending new config to " + r);
    ...
    private boolean relaunchActivityLocked(ActivityRecord r,
        ...
        if (DEBUG_SWITCH) Slog.d(TAG, "Relaunching: " + r
    ...
    boolean handleAppDiedLocked(ProcessRecord app) {
        if (mPausingActivity != null && mPausingActivity.app == app) {
            if (DEBUG_PAUSE || DEBUG_CLEANUP) Slog.d(TAG,
